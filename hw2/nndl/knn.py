...
class KNN(object):
  ...
  def compute_L2_distances_vectorized(self, X):
    ...
    #print((np.sum(X**2,1)[:,np.newaxis]).shape)
    #print((np.sum(self.X_train**2,1)).shape)
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in np.arange(num_test):
	  ...
	  # ================================================================ #
        sort_index = np.argsort(dists[i,:])  #grab the row data array corresponding to a test image's 'distance' from the training images, then sort it
